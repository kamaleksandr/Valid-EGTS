...
uint8_t TEPCompData::Compress(TSubrecord* sbr) {
  ...
  cd.reset((char*) TZLib::Deflate((uint8_t*) data, &size));
  head.cdl = size;
  return cd != 0;
}
...
std::unique_ptr<TSubrecord> TEPCompData::Extract() {
  ...
  std::unique_ptr<char> ptr((char*) TZLib::Inflate((uint8_t*) cd.get(), &size));
  if (ptr)
...
void TInsAccel::SetTime(uint32_t value) { // time in utc in seconds since 2010
...
void TPosData::SetLatitude(double val) {
  if (val < 0) {
    body.flags.lahs = 1;
    val = -val;
  }
  body.lat = val / 90.0 * 0xffffffff;
}
